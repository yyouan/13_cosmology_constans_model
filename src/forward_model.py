...
import forward_model_train as model_train
PACKAGE_PARENT = '..'
...
if len(sys.argv) < 4:
    ...
    print('For example, "./forward_model.py input.a.txt --name test1 --with_graph"')
    exit()
...
def Plot(theory_array,model_array):
        ...
        plt.tight_layout()        
        plt.savefig("graph/"+name+'.forward.model_input.png')
        plt.show()
'''
