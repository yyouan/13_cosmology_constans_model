...
raw_output_array = data[from_data:to_data,(inputLen):]
...
def Plot(theory_array,model_array):
        ...
        plt.tight_layout()        
        plt.savefig("graph/" + name+'.forward.prediction_input.png')
        plt.show()
...
if with_graph == True:
    Plot(raw_output_array, data[:,(inputLen):(inputLen + raw_output_array.shape[1]) ])
